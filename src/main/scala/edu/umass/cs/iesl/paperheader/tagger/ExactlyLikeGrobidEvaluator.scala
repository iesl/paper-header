...
class ExactlyLikeGrobidEvaluator {
  ...
  def evaluate(docs: Seq[Document], writeFiles: Boolean = false, outputDir: String = "results"): (Double, String) = {
    val theResult = makeStringForEval(docs)
    evaluate(theResult, writeFiles = writeFiles, outputDir = outputDir)
  }

  def evaluate(theResult: String, writeFiles: Boolean, outputDir: String): (Double, String) = {
  ...
  def evaluateFromFilename(docs: Seq[Document], tmpFilename: String, writeFiles: Boolean = false, outputDir: String = "results"): (Double, String) = {
    writeFile(docs, tmpFilename)
    evaluate(tmpFilename, writeFiles=writeFiles, outputDir=outputDir)
  }

  def evaluateFromFilename(filename: String, writeFiles: Boolean, outputDir: String): (Double, String) = {
    val report = new StringBuilder()
  ...
  def makeStringForEval(docs: Seq[Document]): String = {
    def grobidLabel(c: String): String =
      if (c.startsWith("B-")) "I-" + c.substring(2)
      else if (c.startsWith("I-")) c.substring(2)
      else c
    val pw = new StringBuilder()
    docs.foreach { doc =>
      doc.tokens.foreach { token =>
        val feats = List(token.string, 0).mkString("\t")
        val gold = grobidLabel(token.attr[HeaderLabel].target.categoryValue)
        val guess = grobidLabel(token.attr[HeaderLabel].categoryValue)
