 ...
 */
class StackedGrobidHeaderTagger(lexicon: StaticLexicons, rlog: Option[Logger], params: Hyperparams, embeddingMap: SkipGramEmbedding = null)
  ...
  def addFeatures2(doc: Document): Unit = {
    ...
    tokenSequence.foreach { token =>
      val fv = token.attr[FeatureVar2]
      fv ++= FeatureExtractor.process(token)
    }

    val vf = (t: Token) => t.attr[FeatureVar2]
